...
- **Context-loaded throwaway turn.** The worker concatenates `AGENTS.md`, `CLAUDE.md`, and a machine-readable snapshot of every unanswered David message into a deterministic on-disk preamble, then pipes it to a fresh `claude -p --dangerously-skip-permissions`.
...
## Known gap (named, not fixed here)

`launchd/com.firstmate.poller.plist` is a user LaunchAgent, so unattended reliability holds only while David is logged in.
