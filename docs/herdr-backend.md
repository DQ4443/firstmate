...
- The same universal requirements as tmux (a verified crew harness, git with native GitHub CLI `gh` authentication, node, treehouse, no-mistakes, chrome-devtools-axi, and lavish-axi); treehouse still provides the worktree, herdr only provides the session.
