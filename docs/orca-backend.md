...
- `git` with native GitHub CLI `gh` authentication, `no-mistakes`, `chrome-devtools-axi`, and `lavish-axi` - the same universal requirements as tmux, minus `tmux` and `treehouse` (Orca replaces both).
